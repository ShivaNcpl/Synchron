...
*/
/*
- (IBAction)startCode:(id)sender {
    if (!_isReading) {
        if ([self startReading]) {
            [_startScan setTitle:@"Stop"];
            [_barCodeNum setText:@"Scanning for QR Code..."];
        }
    }
    else{
        [self stopReading];
        [_startScan setTitle:@"Start!"];
    }
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    ...
    if (metadataObjects != nil && [metadataObjects count] > 0) {
        ...
        if ([[metadataObj type] isEqualToString:AVMetadataObjectTypeQRCode]) {
            ...
            [_barCodeNum performSelectorOnMainThread:@selector(setText:) withObject:[metadataObj stringValue] waitUntilDone:NO];
